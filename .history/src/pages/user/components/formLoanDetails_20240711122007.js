...
// }
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/apiRegistry/get_approved_by_id?id=${guarantor}`, {
        headers: {
  ...
  useEffect(() => {
   ...
   const handleActivate = async e => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/apiRegistryenable_disable?id={guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
