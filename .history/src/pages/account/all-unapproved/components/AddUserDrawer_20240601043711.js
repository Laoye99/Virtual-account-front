...
const SidebarAddUser = props => {
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:9897/vaccount/virtual-account?action=update&id=${guarantor}`, formData, {
        headers: {
      ...
      setCredit('')
      setDebt('')
      router.push('/account/approved-account')
    } catch (error) {
      // Handle errors
  ...
  const handleActivate = async e => {
    ...
    try {
      // Make an HTTP POST request to your endpoint
      const response = await axios.put(`http://localhost:9897/vaccount/virtual-account?action=activate&id=${guarantor}`, {}, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  const handleDeactivate = async e => {
    ...
    try {
      // Make an HTTP POST request to your endpoint
      const response = await axios.put(`http://localhost:9897/vaccount/virtual-account?action=deactivate&id=${guarantor}`,{}, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h5'>Update Virtual Account Config</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
          sx={{
