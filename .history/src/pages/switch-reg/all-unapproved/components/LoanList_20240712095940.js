...
const columns = [
  {
    ...
    headerName: 'ID',
    renderCell: ({ row }) => (
      <LinkStyled href={`/switch-reg/all-unapproved/${row["id"]}`}>
        {`${row["id"]}`}
      </LinkStyled>
    )
  },
  {
    ...
    renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{row["swich_name"] || 0}</Typography>
  },
  {
    flex: 0.1,
    field: 'switch_code',
...
const LoanList = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BASE_URL}/switch/get_pending`, {
        headers: {
