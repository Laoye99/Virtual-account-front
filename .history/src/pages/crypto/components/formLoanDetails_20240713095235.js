...
// }
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/crypto_config/get_approved_by_id?id=${guarantor}`, {
        headers: {
  ...
  useEffect(() => {
   ...
   const handleActivate = async e => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/crypto_config/enable_disable?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  return  (
    ...
    <CardContent
      ...
      <TabContext value={value}>
        ...
        >
          <Tab value='personal-info' label={<span style={{ color: '#f50606' }}>Cypto Configuration Details</span>} />
        </TabList>
 ...
 null) : ( <TableBody>
  ...
  <TableRow>
      ...
      <TableRow>
        ...
        <TableCell>
          {apiData['last_modify_by']}
        </TableCell>
      </TableRow>
      <TableRow>
        <TableCell>Modification Date:</TableCell>
        <TableCell>
          {apiData['last_modify_date']}
        </TableCell>
      </TableRow>
       <TableRow>
      <TableCell>State:</TableCell>
      <TableCell>
      {apiData?.['disabled'] === "false" ? "Enabled" : "Disabled"}
      </TableCell>
    </TableRow>
      </TableBody>)
}
