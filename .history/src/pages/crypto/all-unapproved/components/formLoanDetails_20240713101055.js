...
const FormLayoutsGuarantor = addressID => {
  const guarantor = addressID.addressID
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/crypto_config/get_pending_id?id=${guarantor}`, {
        headers: {
  ...
  const handleApprove = async e => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/crypto_config/approval?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  const handleDecline = async e => {
    ...
    try {
      // Make an HTTP PUT request to your endpoint
      const response = await axios.put(`${BASE_URL}/crypto/approval?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
      ...
      setButtonDisabled(false)
      console.log(response.data)
      toast.success("Channel Declined")
      router.push('/crypto/all-unapproved')
    } catch (error) {
      // Handle errors
      toast.error('Please try again')
      console.error('Error submitting form', error)
  ...
  return  (
    ...
    <CardContent
      ...
      <TabContext value={value}>
        ...
        >
          <Tab value='personal-info' label={<span style={{ color: '#f50606' }}>Crypto Configuration Details</span>} />
        </TabList>
            ...
            <Table sx={{ minWidth: 200 }}>
{
 ...
 null) : ( <TableBody>
  ...
  <TableRow>
      ...
      <TableRow>
      <TableCell>Private Keyfile:</TableCell>
      <TableCell>
      {apiData?.['its_private_keyfile']}
      </TableCell>
    </TableRow>
    <TableRow>
      <TableCell>Public Keyfile:</TableCell>
      <TableCell>
      {apiData?.['its_public_keyfile']}
      </TableCell>
    </TableRow>
    <TableRow>
      <TableCell>Switch Public Keyile:</TableCell>
      <TableCell>
      {apiData?.['switch_public_keyfile']}
      </TableCell>
    </TableRow>
      <TableRow>
