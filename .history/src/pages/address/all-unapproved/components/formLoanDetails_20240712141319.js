...
const FormLayoutsGuarantor = addressID => {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/thirdPartyAddr/get_pending_id?id=${guarantor}`, {
        headers: {
  ...
  const handleApprove = async e => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/thirdPartyAddr/approval?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  const handleDecline = async e => {
    ...
    try {
      // Make an HTTP PUT request to your endpoint
      const response = await axios.put(`${BASE_URL}/switch/approval?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
      ...
      router.push('/switch-reg/all-unapproved')
    } catch (error) {
      // Handle errors
      toast.error('Please try again')
      console.error('Error submitting form', error)
  ...
  return  (
    ...
    <CardContent
      ...
      <Button
          component={Link}
          variant='contained'
          href='/switch-reg/all-unapproved'
