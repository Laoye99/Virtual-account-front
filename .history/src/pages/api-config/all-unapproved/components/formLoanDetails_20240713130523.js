...
const FormLayoutsGuarantor = accountID => {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/apiConfig/get_pending_by_id?id=${guarantor}`, {
        headers: {
  ...
  const handleApprove = async e => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/apiConfig/approval?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  return  (
    ...
    <CardContent
      ...
      <Button
          component={Link}
          variant='contained'
          href='/api-config/all-unapproved'
