...
// }
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/apiConfig/get_approved_by_id?id=${guarantor}`, {
        headers: {
  ...
  useEffect(() => {
   ...
   const handleActivate = async e => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/apiConfig/enable_disable?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  return  (
    ...
    <CardContent
      ...
      <TabContext value={value}>
        ...
        >
          <Tab value='personal-info' label={<span style={{ color: '#f50606' }}>Third Party API Configuration Details</span>} />
        </TabList>
            ...
            <Table sx={{ minWidth: 200 }}>
{
 ...
 null) : ( <TableBody>
  <TableRow>
      ...
      <TableRow>
        <TableCell>Tsq Url:</TableCell>
        <TableCell>
          {apiData?.['tsq_url']}
        </TableCell>
      </TableRow>
      <TableRow>
        <TableCell>Trf Name:</TableCell>
        <TableCell>
          {apiData?.['app_name']}
        </TableCell>
      </TableRow>
