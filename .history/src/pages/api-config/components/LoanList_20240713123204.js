...
const columns = [
  {
    ...
    headerName: 'ID',
    renderCell: ({ row }) => (
      <LinkStyled href={`/api-config/${row["id"]}`}>
        {`${row["id"]}`}
      </LinkStyled>
    )
  },
  ...
  },
  {
    ...
    minWidth: 120,
    headerName: 'TSQ-URL',
    renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{row["tsq_url"] || 0}</Typography>
  },
  {
    flex: 0.1,
    field: 'trf_url',
    minWidth: 120,
    headerName: 'TRF-URL',
    renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{row["trf_url"] || 0}</Typography>
  },
  {
    flex: 0.1,
    field: 'created_by',
...
const LoanList = () => {
  ...
  >
   New Third Party Api Configuration
