...
const LoanList = (switchID) => {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/switch/uploadfile?module=contra_account&category=approved&fetch-content=true&id=${guarantor}`, {
        headers: {
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/switch/uploadfile?module_name=contra_account&id=${guarantor}`, null, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
      ...
      setMessage("")
      router.push('/fee-configuration/approved-upload')
