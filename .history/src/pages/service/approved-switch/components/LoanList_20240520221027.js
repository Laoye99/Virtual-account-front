...
const columns = [
  {
    ...
    renderCell: ({ row }) => <LinkStyled href={`/service/approved-service/${row["service-provider-id"]}`}>{`#${row["service-provider-id"]}`}</LinkStyled>
  },
  {
    flex: 0.1,
  ...
  },
  {
    ...
    renderCell: ({ row }) => (
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        <Tooltip title='View'>
          <IconButton size='small' component={Link} href={`/service/approved-service/${row["service-provider-id"]}`}>
