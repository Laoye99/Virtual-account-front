...
const FormLayoutsGuarantor = serviceID => {
  ...
  function findServiceProviderById(serviceProviderId) {
    return data.data.find(item => item['service-provider-id'] === serviceProviderId);
}
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`http://localhost:9897/vaccount/service-provider?mc-status=unapproved&id=${guarantor}`, {
        headers: {
      ...
      console.log(response.data?.data['service-provider-id'={guarantor}])
    } catch (error) {
  ...
  const handleApprove = async e => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:9897/vaccount/approve-service?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
