...
const FormLayoutsGuarantor = serviceID => {
  ...
  const handleApprove = async e => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:9897/vaccount/service-provider/approve-service?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  const handleDecline = async e => {
    ...
    try {
      // Make an HTTP POST request to your endpoint
      const response = await axios.post(`http://localhost:9897/vaccount/service-provider?mc-status=unapproved?id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
