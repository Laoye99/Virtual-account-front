...
const FormLayoutsGuarantor = serviceID => {
  ...
  const handleActivate = async e => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:9897/switch/service-provider?action=activate&id=${guarantor}`, formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
  ...
  const handleDeactivate = async e => {
    ...
    try {
      // Make an HTTP POST request to your endpoint
      const response = await axios.put(`http://localhost:9897/switch/service-provider?action=deactivate&id=${guarantor}`,formData, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
