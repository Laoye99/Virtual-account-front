...
const LoanList = (switchID) => {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.get(`${BASE_URL}/switch/uploadfile?module=inst_route&category=approved&fetch-content=true&id=${guarantor}`, {
        headers: {
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/switch/uploadfile?module_name=inst_route&id=${guarantor}`, null, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
      ...
      setMessage("")
      router.push('/router/approved-upload')
