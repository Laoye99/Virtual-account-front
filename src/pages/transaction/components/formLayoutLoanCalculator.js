...
console.log(formattedEndDate, formattedStartDate)
  ...
  const onSubmit2 = async (searchValue) => {
    // Disable the button
    setButtonDisabled(true)
    ...
    try {
      // Make an HTTP POST request to your endpoint
      const response = await axios.get(`${BASE_URL}/switch/transactions?switch-type=${loantype}&session-id=${searchValue}`, {
        headers: {
          Authorization: `Bearer ${storedToken}`,
          'Content-Type': 'application/json',
          "ngrok-skip-browser-warning": "http://localhost:3000/"
...
<Grid item xs={12} sm={3}>
    ...
    <MenuItem value='up_outflow'>up_outflow</MenuItem>
	  ...
	  <MenuItem value='up_inflow'>up_inflow</MenuItem>
    </CustomTextField>
  </Grid>
                <Grid item xs={12} sm={3}>
