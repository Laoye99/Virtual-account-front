...
const columns = [
  {
    ...
    renderCell: ({ row }) => (
      <LinkStyled href={`/account/${row["service-provider"]}`}>
  ...
  },
  {
    ...
    field: 'acct-enq-url',
    minWidth: 120,
    headerName: 'ACCT ENQ URL',
    renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{row["acct-enq-url"] || 0}</Typography>
  },

  {
    ...
    field: 'acct-prefix',
    minWidth: 120,
    headerName: 'ACCT PREFIX',
    renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{row["acct-prefix"] || 0}</Typography>
  },
  {
    flex: 0.1,
    field: 'created-by',
  ...
  },
  {
    ...
    minWidth: 120,
    headerName: 'Action',
    renderCell: ({ row }) => <Typography sx={{ color: 'text.secondary' }}>{row["action"] || 0}</Typography>
  }
]
...
const LoanList = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:9897/vaccount/virtual-account?mc-status=unapproved', {
        headers: {
  ...
  return data ? (
    <Card>
      ...
      <DataGrid
        ...
        getRowId={(row) => row['service-provider']}
      />
